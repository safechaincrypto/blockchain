...
const string strMessageMagic = "safechain Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // safechain: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // safechain: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
if (pindexBest->nHeight < 10){
        int64_t nSubsidy = 60000000 * COIN; // XZURMO SUPPLY
...
// miner's coin stake reward based on coin age spent (coin-days) FIXED PoS SUBSIDY REWARD 
...
// safechain: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // safechain: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // safechain: clean up wallet after disconnecting coinstake
...
if(IsProofOfWork())
    ...
    if (IsProofOfStake())
    {
        // safechain: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // safechain: track money supply and mint amount info
...
// safechain: total coin age spent in transaction, in the unit of coin-days.
...
// safechain: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // safechain: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // safechain: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // safechain: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // safechain: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // safechain: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // safechain: check proof-of-stake
    ...
    // safechain: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // safechain: getblocks may not obtain the ancestor block rejected
    ...
    // safechain: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "safechain", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "safechain born a new era";
        CTransaction txNew;
        txNew.nTime = 1627381879;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x72e8f05f46c59fe81db6648bb7297e780aac44b3e3dda029b99a8d465b8ae3ca"));
        block.print();
        ...
        // safechain: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // safechain: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // safechain: tell downloading node about the latest block if it's
