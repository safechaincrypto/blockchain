...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.10 * COIN;	// 2% Interest 
static const int64_t COIN_YEAR_REWARD = 10 * CENT; //(output to console will be updated)
static const int MODIFIER_INTERVAL_SWITCH = 100; // PoS begin 

#define FOUNDATION "BHURxKsDC22xSe72MyqEp6Bmx7j9eCW1um"
...
public:
    ...
    {
        // safechain: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // safechain: get transaction coin age
...
public:
    ...
    // safechain: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // safechain: two types of block: proof-of-work or proof-of-stake
    ...
    // safechain: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // safechain: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // safechain: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // safechain: block index flags
    enum  
    {
